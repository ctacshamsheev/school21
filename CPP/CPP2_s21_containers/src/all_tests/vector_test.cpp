...
TEST(Vector_tests, insert) {
  ...
  s21::Vector<int> C;
  std::vector<int> D;
  C.insert(C.begin(), 777);
  D.insert(D.begin(), 777);
  EXPECT_EQ(C.size(), D.size());
  EXPECT_EQ(C.capacity(), D.capacity());
  for (size_t i = 0; i < C.size(); i++) {
    EXPECT_EQ(C[i], D[i]);
...
TEST(Vector_tests, emplace) {
  ...
  s21::Vector<int> A(10);
  std::vector<int> B(10);
  A.emplace(A.begin(), 1, 2, 77);
  B.emplace(B.begin(), 1, 2, 77);
  EXPECT_EQ(A.size(), B.size());
  EXPECT_EQ(A.capacity(), B.capacity());
  for (size_t i = 0; i < A.size(); i++) {
    EXPECT_EQ(A[i], B[i]);
  ...
  s21::Vector<int> C;
  std::vector<int> D;
  C.emplace(C.begin(), 1, 2, 77);
  D.emplace(D.begin(), 1, 2, 77);
  EXPECT_EQ(C.size(), D.size());
  EXPECT_EQ(C.capacity(), D.capacity());
  for (size_t i = 0; i < C.size(); i++) {
    EXPECT_EQ(C[i], D[i]);
  ...
  s21::Vector<int> E(values);
  std::vector<int> F(values);
  C.emplace(--(C.end()), 1, 2, 77);
  D.emplace(--(D.end()), 1, 2, 77);
  EXPECT_EQ(E.size(), F.size());
  EXPECT_EQ(E.capacity(), F.capacity());
  for (size_t i = 0; i < E.size(); i++) {
    EXPECT_EQ(E[i], F[i]);
...
TEST(Vector_tests, emplace_back) {
  ...
  s21::Vector<int> A(10);
  std::vector<int> B(10);
  A.emplace_back(1, 2, 77);
  B.emplace_back(1, 2, 77);
  EXPECT_EQ(A.size(), B.size());
  EXPECT_EQ(A.capacity(), B.capacity());
  for (size_t i = 0; i < A.size(); i++) {
    EXPECT_EQ(A[i], B[i]);
  ...
  s21::Vector<int> C;
  std::vector<int> D;
  C.emplace_back(1, 2, 77);
  D.emplace_back(1, 2, 77);
  EXPECT_EQ(C.size(), D.size());
  EXPECT_EQ(C.capacity(), D.capacity());
  for (size_t i = 0; i < C.size(); i++) {
    EXPECT_EQ(C[i], D[i]);
